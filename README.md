...
以下是 GitHub 上 Star 较多、比较有代表性的数学建模开源仓库，和本项目的区别：

### [Visualize-ML/Book7](https://github.com/Visualize-ML/Book7_Visualizations-for-Mathematical-Modeling) — 数学建模可视化方法大全

一本讲数学建模可视化的书，画图方面很专业。但它是一本书，不是工具——看完知道怎么画图，但你还得自己写代码、自己找数据、自己写论文。

本项目的不同：**内置 16 种图表的绘制工具**（`skill/scripts/plot_helpers.py`），代码求解和画图是流水线的一部分，跑完代码图就出了，不需要再翻书查参数。

### [zhanwen/MathModel](https://github.com/zhanwen/MathModel) — 数学建模算法汇总

算法教材和 LaTeX 源码，学算法原理很好。但没有 Python 可运行代码，也没有 AI 集成。

本项目的不同：**每个算法对应可运行脚本**（见 `samples/` 下的 `code/` 目录），而且由子智能体根据具体赛题自动选择算法，不是把几十个算法列出来让你自己挑。

### [BetterBench/Math_modeling](https://github.com/BetterBench/Math_modeling) — 学习资源合集

资料整理得比较全（书、课、工具链接），适合初学者入门了解。但本质是链接集合，不产出任何内容。

本项目的不同：**本身就是一个内容生产系统**——输入赛题，输出 PDF 论文。同时附带两个完整案例（国赛 A 题 + HiMCM B 题）的全流程记录和最终论文，可以直接参考。

### [qiqiqiqishu/Mathematical-Modeling-Competition](https://github.com/qiqiqiqishu/Mathematical-Modeling-Competition) — 竞赛经验与代码分享

个人参赛经验的分享，含一些代码片段和学习路径。对了解竞赛流程有帮助，但代码偏零散，没有形成系统。

本项目的不同：**完整流水线**，从审题到审稿每个阶段都有固定产出和门禁验证。代码、数据、图表、论文四位一体互相可追溯。

### [luoguofeng0406/Mathematical-Modeling](https://github.com/luoguofeng0406/Mathematical-Modeling) — 常用算法 Python 实现

常见建模算法的 Python 实现（插值、拟合、规划、微分方程等），代码可作为备查参考。但没有针对具体赛题的解题流程，也没有论文输出。

本项目的不同：**算法实现 + 解题流程 + 论文产出打包在一起**。不是给你一个插值工具箱，而是针对你的赛题跑完整个流程。

---

总体来看，上述项目聚焦在**"教你怎么做"**（教程、资料、工具链接），本仓库聚焦在**"帮你做出来"**（自动化流水线 + 完整案例）。两类项目互补，不冲突。

| | 可视化书 | 算法书 | 资料合集 | 经验分享 | 算法代码 | **本仓库** |
| -- | :--: | :--: | :--: | :--: | :--: | :--: |
| 教程/资料 | ✅ | ✅ | ✅ | ✅ | ✅ | 含 2 个完整案例 |
| 可运行代码 | ❌ | ❌ | ❌ | 部分 | ✅ | ✅ + 自动化 |
| 数据获取 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ 联网搜 |
| 论文输出 | ❌ | ✅ LaTeX 源码 | ❌ | ❌ | ❌ | ✅ 自动编译 PDF |
| AI 编排 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ 8 智能体协作 |
| 自动审稿 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ 3 角色对抗 |

**这个项目适合谁？**
- 刚接触数学建模，想看看一个完整流程长什么样
- 有思路但写代码和论文太花时间，想要一套加速工具
- 想用 AI 辅助建模，但对单次 ChatGPT 问答的效果不满意
