...
## v2.2.1 (2026-07-31) — 结构清理 + Token 经济 + 一致性修复

### 一致性修复（P0）
- **版本号统一**：SKILL.md 全文 v2.1 → v2.2，删除所有"v2.1 新增"残留表述；08 手册同步
- **删除重复内容**：阶段 7 验证表在 SKILL.md 出现两次，删除第二份
- **硬编码路径清除**：样本代码 7 个 solve_q*.py 的 `sys.path.insert(0, r"C:\Users\HUAWEI\...")` 改为 `os.path.dirname(__file__)` 相对推导；05-visualization.md 模板改为 `${SKILL_ROOT}`/相对推导；SKILL.md 环境说明不再写死绝对路径
- **质检只读声明修正**：Subagent "只读质检"改为 best-effort（提示词+权限尽力隔离，不作硬性保证），避免不可实现的承诺；workflow 评审提示词补充"只读评审、不得修改文件"

### Agent 权限补全（P0）
- `mm-reviewer` 补 `Bash` 工具（A2/A3 反模式扫描可跑验证脚本）

### Token 经济（P1）
- **Fast Path**：审题后判定题目复杂度，简单题可跳过阶段 1.5 文献综述、阶段 7 审稿降为 1 轮；核心门禁与人工在环不可省
- **相邻门禁合并**：P2+A1 由同一 mm-verifier 一次派发输出双报告
- **SKILL.md 精简**：内嵌阶段 1-7 验证详情表移除，统一指向 08-stage-verification.md；SKILL.md 只保留门禁速查表 + 关键规则

### 指标与手册（P2）
- **字数指标改为维度覆盖**：审题报告从"≥2000 字"改为"§1.1–§1.7 七维度每维 ≥ 150 字"（01/08/SKILL.md 三处同步）
- **06-writing.md 扩充**：新增美赛专属要求（Summary Sheet/页数上限/英文规范）+ 语言与逻辑规范
- **09-innovation-playbook.md 扩充**：新增创新呈现方式表 + 创新投入产出权衡 + 常见误区

### 样本澄清（P3）
- 确认 samples/2025_HiMCM_Problem_B 为官方公开赛题演示示例（非真实参赛作品），新增 README 标注性质

### Python 环境
- SKILL.md 环境说明更新：Python 一律用 `C:/Users/HUAWEI/AppData/Local/Programs/Python/Python311/python.exe`（用户指定）
