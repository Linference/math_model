...
| `mm-verifier` | 交叉验证：数值/量纲/边界/自洽性检查 | 4-7 |
| `mm-reasoner` | 数学推理：公式推导审计、补全未证断言 | 2,4,7 |
...
- **每阶段验证门禁**：执行→验证→确认，验证不通过不得前进（阶段4算法审计为最关键门禁）
...
│   ├── verify_results.py          # 自动交叉验证（代码↔论文数字）
...
- **审题即宪法**：阶段 1 审题结论是后续所有阶段的最高指导，阶段 2-6 必须显式引用
- **算法即根基**：阶段 4 是流水线根基——算法错则全盘错，验证不通过绝不允许前进
...
- **版本即安全网**：每次编译自动留存 PDF 版本，审稿每轮可追溯
