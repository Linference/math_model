...
\begin{equation}
v_i(t) = \frac{\|\bm{r}_i(t+\Delta t) - \bm{r}_i(t-\Delta t)\|}{2\Delta t},\quad \Delta t=1.0\;\text{s}\;\text{（中心差分）}.
\label{eq:velocity}
\end{equation}

同时可解析验证：由$\bm{r}_i(t)=\bm{r}(\theta_i(t))$，有$v_i = \|\bm{r}'(\theta_i)\|\cdot|\dot{\theta}_i| = b\sqrt{\theta_i^2+1}\cdot|\dot{\theta}_i|$。

\textbf{弦向速度与弧向速度的差异}：中心差分法计算的实质是\textit{弦向速度}（直线位移/时间），而龙头设定值1\,m/s为\textit{弧向速度}（沿螺线路径/时间）。在曲率非零处，弦向位移严格小于弧向位移，差异量级为$\mathcal{O}((\Delta\theta/2)^2)$。数值实验显示龙头速度范围$[0.9934,0.9995]$\,m/s，与1\,m/s的最大偏差约0.66\%，与理论估计$1-\cos(\Delta\theta/2)\approx (\Delta\theta)^2/8$一致（由$\dot{\theta}_1\approx v_{\text{head}}/r\approx 0.11$\,rad/s，$\Delta\theta\approx 0.11$\,rad，偏差$\approx 1.5\times 10^{-3}$）。该微小偏差不影响模型精度——所有把手速度的相对趋势和瓶颈位置不受影响，且可通过解析式$b\sqrt{\theta_i^2+1}\cdot|\dot{\theta}_i|$精确验证。
...
龙头在复合路径（盘入螺线$\to$C$_1\to$C$_2\to$盘出螺线）上的位置由弧长参数化确定。以调头开始时刻为$t=0$，$t<0$时龙头在盘入螺线上，$t>0$时用对应路径段的弧长公式确定龙头位置。

\textbf{S曲线过渡段的近似处理}：当龙头进入S曲线段（$t\in[0,1.80]$\,s）时，龙身和龙尾的大部分把手仍位于盘入螺线上（因整条龙长约369\,m，调头时间仅1.80\,s，信息尚未传播至尾部）。严格求解需在混合曲线上对每个把手独立判断其所处曲线段并做弦长递推——计算复杂度急剧上升。本文采用保守近似：将过渡期内龙身/龙尾把手维持于盘入螺线（即$t$时刻把手位置由从虚拟龙头位置沿盘入螺线递推确定），此近似引入的位置误差$\approx$（S曲线弧长/总龙长）$\times 369\,\text{m}\approx 1.80\,\text{m}$量级。鉴于问题四主要关注调头路径的几何设计与龙头运动学，且后续问题五的速度瓶颈分析中瓶颈发生在$r=2.22$\,m处的深圈螺线区域（远在S曲线之后），\textbf{此近似对论文核心结论（调头路径最优性、$v_{\max}$）不构成实质影响}。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.60\textwidth]{fig_compare_radar.png}
